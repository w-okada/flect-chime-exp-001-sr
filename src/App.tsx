...
function App() {
    ...
    const { meetingSession1, meetingSession2, stateUpdateTime} = useChimeClientPair({})
    ...
    const [ outputViewTypeKey, setOutputViewTypeKey] = useState(Object.keys(outputViewTypes)[0])
    ...
    const setLayout2 = () =>{
        ...
        const inputElem   =  (inputMedia.mediaType === "IMAGE" || inputMedia.mediaType === "IMAGE_URL")? document.getElementById("input_img") as HTMLImageElement : document.getElementById("input_video") as HTMLVideoElement
        const org = document.getElementById("original") as HTMLCanvasElement
        const sent = document.getElementById("to_be_sent") as HTMLCanvasElement
        const recv = document.getElementById("received_video") as HTMLVideoElement
        const dst_espcn = document.getElementById("output_espcn") as HTMLCanvasElement
        const dst_cubic = document.getElementById("output_cubic") as HTMLCanvasElement
        const dst_canvas = document.getElementById("output_canvas") as HTMLCanvasElement
    ...
    useEffect(()=>{
        ...
        const org = document.getElementById("original") as HTMLCanvasElement
        const recv = document.getElementById("received_video") as HTMLVideoElement
        const recv_canvas = document.getElementById("receive-canvas") as HTMLCanvasElement
        ...
        const exe = async () => {
            ...
            for(let i=0; i<TARGET.length;i++){
                ...
                console.log(path, Math.ceil(ratio * img.width), Math.ceil(ratio * img.height));

                const sizes = [[480, 360], [320, 240], [240, 180]]
                const countStr = (i+OFFSET).toString().padStart(4, "0")
                const h_name = `${countStr}.png`
                for(let j=0; j<sizes.length;j++){
                    orgCtx.drawImage(img, 0, 0, Math.ceil(ratio * img.width), Math.ceil(ratio * img.height))
                    const [w, h] = sizes[j]
                    sent.width = w
                    sent.height = h
                    recv.width = w
    ...
    useEffect(()=>{
        ...
        const exe = async () => {
            ...
            // for(let i=0; i<5;i++){
                ...
                // (4) Calc Score
                if(imageScore){
                    imageScore.setImage(org, dst as HTMLCanvasElement, {useSimd:useSIMD})
                    const psnr = imageScore.psnr({useSimd:useSIMD})
                    const { mssimR, mssimG, mssimB, mssimA} = imageScore.mssim({useSimd:useSIMD});
                    console.log(`psnr: ${psnr.toFixed(2)} mssim:${mssimR.toFixed(2)},${mssimG.toFixed(2)},${mssimB.toFixed(2)}`)
                    psnrs.push(psnr)
                    mssimRs.push(mssimR)
                    mssimGs.push(mssimG)
                    mssimBs.push(mssimB)

                    const [psnr_avr, mssimR_avr, mssimG_avr, mssimB_avr] = [psnrs, mssimRs, mssimGs, mssimBs].map((arr)=>{
                        return arr.reduce((prev,current)=>{return prev+current},0) / arr.length
                    })
                    console.log(`psnr: ${psnr_avr.toFixed(2)} mssim:${mssimR_avr.toFixed(2)},${mssimG_avr.toFixed(2)},${mssimB_avr.toFixed(2)}`)
                    const eval_info = document.getElementById("eval_info") as HTMLDivElement
                    eval_info.innerText=`avr psnr: ${psnr_avr.toFixed(2)} mssim:${mssimR_avr.toFixed(2)},${mssimG_avr.toFixed(2)},${mssimB_avr.toFixed(2)}`
                }
    ...
    useEffect(() => {
        ...
        const src   =  (inputMedia.mediaType === "IMAGE" || inputMedia.mediaType === "IMAGE_URL")? document.getElementById("input_img") as HTMLImageElement : document.getElementById("input_video") as HTMLVideoElement
        const org = document.getElementById("original") as HTMLCanvasElement
        const sent = document.getElementById("to_be_sent") as HTMLCanvasElement
        ...
        const render = async () => {
            ...
            if(src.width === 0 || src.height === 0 || sent.width === 0 || sent.height === 0){
            ...
            const inputWidth  = (inputMedia.mediaType === "IMAGE" || inputMedia.mediaType === "IMAGE_URL")? (src as HTMLImageElement).naturalWidth  : (src as HTMLVideoElement).videoWidth 
            const inputHeight = (inputMedia.mediaType === "IMAGE" || inputMedia.mediaType === "IMAGE_URL")? (src as HTMLImageElement).naturalHeight : (src as HTMLVideoElement).videoHeight             
    ...
    useEffect(() => {
        ...
        const src = document.getElementById("input_img") ? document.getElementById("input_img") as HTMLImageElement : document.getElementById("input_video") as HTMLVideoElement
        const org = document.getElementById("original") as HTMLCanvasElement
        const sent = document.getElementById("to_be_sent") as HTMLCanvasElement
        const recv = document.getElementById("received_video") as HTMLVideoElement
        const dst = document.getElementById("output_espcn") as HTMLCanvasElement
        ...
        const canvas_info = document.getElementById("canvas_info") as HTMLDivElement
        // Hide unsed element
        ...
        const getPredictionImage = async(workerProps:WorkerProps, sent:HTMLCanvasElement, recv:HTMLVideoElement, interpolationType:number) =>{
            ...
            let prediction
            if(applyToSrc){
                prediction = await workerProps.manager.predict(sent!, workerProps.params)
            }else{
                prediction = await workerProps.manager.predict(recv!, workerProps.params)
            }
            if (!prediction) {
            }else{
                try{
        ...
        const render = async () => {
            ...
            if(src.width === 0 || src.height === 0 || sent.width === 0 || sent.height === 0 || recv.width === 0 || recv.height === 0){
                renderRequestId = requestAnimationFrame(render)
                return
            }
            if (workerProps) {
                if(workerProps.params.scaleFactor===1){  // (1) Scale Factor is 0 => copy recv to dst
                    ...
                    if(calcScore && imageScore){
                        ...
                        const psnr = imageScore.psnr({useSimd:useSIMD})
                        const { mssimR, mssimG, mssimB, mssimA} = imageScore.mssim({useSimd:useSIMD});
                        (info as HTMLDivElement).innerHTML = `COPY <br/> psnr: ${psnr.toFixed(2)}<br/> mssim:${mssimR.toFixed(2)},${mssimG.toFixed(2)},${mssimB.toFixed(2)}`
                    }else{
                        (info as HTMLDivElement).innerHTML = `COPY <br/> `
                    }
                }else if (outputViewTypes[outputViewTypeKey] === OutputViewType.ESPCN){ // (2) ESPCN
                    const dstCtx = dst.getContext("2d")!
                    const image = await getPredictionImage(workerProps, sent, recv, InterpolationType.INTER_ESPCN)
                    if(image){
                        dstCtx.putImageData(image, 0, 0)
                    }
                    info.innerHTML = `ESPCN <br/> `

                }else if(outputViewTypes[outputViewTypeKey] === OutputViewType.CUBIC){ // (3) Cubic
                    const dstCtx = dst.getContext("2d")!
                    const image = await getPredictionImage(workerProps, sent, recv, InterpolationType.INTER_CUBIC)
                    if(image){
                        dstCtx.putImageData(image, 0, 0)
                    }
                    info.innerHTML = `CUBIC <br/> `

                }else if(outputViewTypes[outputViewTypeKey] === OutputViewType.CANVAS){ // (3) Canvas
                    const dstCtx = dst.getContext("2d")!
                    dstCtx.drawImage(recv, 0, 0, dst_canvas.width, dst_canvas.height)
                    info.innerHTML = `CANVAS <br/> `

                }else if(outputViewTypes[outputViewTypeKey] === OutputViewType.ALL){ // (3) Canvas
                    const dstCtx       = dst.getContext("2d")!
                    const dstCubicCtx  = dst_cubic.getContext("2d")!
                    const dstCanvasCtx = dst_canvas.getContext("2d")!
                    const image = await getPredictionImage(workerProps, sent, recv, InterpolationType.INTER_ESPCN)
                    const imageCubic = await getPredictionImage(workerProps, sent, recv, InterpolationType.INTER_CUBIC)
                    if(image){
                        dstCtx.putImageData(image, 0, 0)
                    }
                    if(imageCubic){
                    ...
                    dstCanvasCtx.drawImage(recv, 0, 0, dst_canvas.width, dst_canvas.height)

                    if(imageScore){
                        [[info, dst, "ESPCN"], [cubic_info, dst_cubic, "CUBIC"], [canvas_info, dst_canvas, "CANVAS"] ].forEach(([info, dst, label])=>{
                            if(calcScore){
                                imageScore.setImage(org, dst as HTMLCanvasElement, {useSimd:useSIMD})
                                console.log(`SCORE::::: ${org.width} ${org.height} ${(dst as HTMLCanvasElement).width} ${(dst as HTMLCanvasElement).height}`)
                                const psnr = imageScore.psnr({useSimd:useSIMD})
                                const { mssimR, mssimG, mssimB, mssimA} = imageScore.mssim({useSimd:useSIMD});
                                    (info as HTMLDivElement).innerHTML = `${label as string} <br/> psnr: ${psnr.toFixed(2)}<br/> mssim:${mssimR.toFixed(2)},${mssimG.toFixed(2)},${mssimB.toFixed(2)}`
                            }else{
                                (info as HTMLDivElement).innerHTML = `${label as string} <br/> `
                            }
                        })
                    }
        

                }else if(outputViewTypes[outputViewTypeKey] === OutputViewType.SPLIT_ESPCN_CUBIC){
                    const dstCtx       = dst.getContext("2d")!
                    const image        = await getPredictionImage(workerProps, sent, recv, InterpolationType.INTER_ESPCN)
                    const imageCubic   = await getPredictionImage(workerProps, sent, recv, InterpolationType.INTER_CUBIC)
                    if(imageCubic){
                    ...
                    if(image){
                        dstCtx.putImageData(image, 0, 0, 0, 0, splitProps.splitX, dst.height)
                    }
                    dstCtx.beginPath()
                ...
                }else if(outputViewTypes[outputViewTypeKey] === OutputViewType.SPLIT_ESPCN_CANVAS){
                    const dstCtx       = dst.getContext("2d")!
                    const image        = await getPredictionImage(workerProps, sent, recv, InterpolationType.INTER_ESPCN)
                    dstCtx.drawImage(recv, 0, 0, dst_canvas.width, dst_canvas.height)                    
                    if(image){
            ...
            const input_info = document.getElementById("input_info") as HTMLDivElement
            const send_info  = document.getElementById("send_info") as HTMLDivElement
            const recieve_info = document.getElementById("receive_info") as HTMLDivElement
            const recover_info = document.getElementById("recover_info") as HTMLDivElement
    ...
    }, [workerProps, inputMedia, inputSize, imageScore, startDataGen, applyToSrc, calcScore, outputViewTypeKey, splitX]) // eslint-disable-line
        ...
        <div className="App">
            <div style={{ display: "flex", flexDirection: "row" }} >
                <div style={{ display: "flex", flexDirection: "column" }}>
                    ...
                    <DropDown title="OutputViewType" current={outputViewTypeKey} onchange={setOutputViewTypeKey} options={outputViewTypes} />
                    

                    <Toggle title="onLocal" current={onLocal} onchange={setOnLocal} />
                    <SingleValueSlider title="inputSize(w)" current={inputSize} onchange={setInputSize} min={240} max={720} step={240} />
                    ...
                    <Toggle title="calcScore" current={calcScore} onchange={setCalcScore} />

                    {/* <Toggle title="setStartDataGen" current={startDataGen} onchange={setStartDataGen} /> */}
                    <Toggle title="setStartEval" current={startEval} onchange={setStartEval} />
                ...
                <div style={{ display: "flex", flexDirection: "row"}}>
                    ...
                    <div   style={{ display: "flex", flexDirection: "column", padding:"4px", alignItems:"center"}}>
                        <div>
                            (2) Sent image(Low Res)
                        </div>
                        <div id="send_info" />
